...
// Primeiro, defina a interface para despesas recorrentes
interface DespesaRecorrente {
  id?: number;
  descricao: string;
  valor: number;
  vencimento: string | Date;  // pode ser string quando vem da API ou Date depois de convertido
  frequencia: string;
}

// Adicione esta interface junto com as outras no topo do arquivo
interface Despesa {
  id?: number;
  descricao: string;
  valor: number;
...
// Primeiro, defina a interface para despesas divididas
...
// Atualize a interface dos dados financeiros
interface DadosFinanceiros {
  saldoAtual: number;
  limiteMensal: number;
  gastosMes: number;
  despesasRecorrentes: DespesaRecorrente[];
  despesasDivididas: DespesaDividida[]; // Agora usando a interface correta
}
...
export default function Component() {
  ...
  const carregarDados = async () => {
    try {
      ...
      console.log('Dados das despesas:', dadosDespesas);
      ...
      const gastosMes = dadosDespesas
        .filter(despesa => {
          const dataDespesa = new Date(despesa.data);
          return dataDespesa.getMonth() === dataAtual.getMonth() &&
                 dataDespesa.getFullYear() === dataAtual.getFullYear();
        })
        .reduce((total, despesa) => total + Number(despesa.valor), 0);

      console.log('Total de gastos do mês:', gastosMes);

      // Atualiza o estado com todos os dados
      const dadosAtualizados = {
        saldoAtual: dadosLimite.limiteMensal - gastosMes,
  ...
  useEffect(() => {
    ...
    if (notifications.upcomingExpenses) {
      ...
      dados.despesasRecorrentes.forEach(despesa => {
        // Agora o TypeScript sabe que vencimento existe
